...
def copy_upto_schemaless_pathes(record, schemaless_pathes):
    if not schemaless_pathes:
        return record

    schemaless_pathes = [path for path in schemaless_pathes if path]
    new_pathes = []
    for path in schemaless_pathes:
        root = path[0]
        children = path[1:]
            ...
            if isinstance(record, typing.Mapping):
                for k, v in record.items():
                    if v:
                        v = copy(v)
                        elements.append(v)
                        record.__setitem__(k, v)
            if isinstance(record, typing.Iterable):
                for idx, i in enumerate(record):
                    if i:
                        i = copy(i)
                        elements.append(i)
                        record.__setitem__(idx, i)
        else:
            if isinstance(record, typing.Mapping):
                if root in record and record[root]:
                    v = copy(record[root])
                    elements.append(v)
                    record.__setitem__(root, v)
            else:
                raise NotImplementedError("Access to list elements not implemented")
        if children:
            new_pathes.append((elements, children))
    for elements, children in new_pathes:
        for o in elements:
            copy_upto_schemaless_pathes(o, [children])
    return record
...
def to_schemaless_avro(o, schema_gen=None, schemaless_pathes=[], types_to_str=(), recurse=False):
    """
    Converts a python nested data structure and returns a data structure
    in schemaless_avro format. 
    schemaless_avro format is conforming to the avro schema generated by schemaless_avro_schema()
    For fastavro::
        fastavro.writer(
            out_stream,
            schema.parse_schema(schemaless_avro_schema),
            (to_schemaless_avro(record) for record in input_records))
    See also::
    ...
    if schemaless_pathes:
        o = copy_upto_schemaless_pathes(o, schemaless_pathes)
        for record_with_setter in get_schemaless_records(o, schemaless_pathes):
            ...
            record = to_schemaless_avro(record, recurse=True)
    ...
    if isinstance(o, Mapping):
        return {'_': {
            k: to_schemaless_avro(v, types_to_str, recurse=recurse)
            for k, v in o.items()}
        }

    if isinstance(o, Iterable):
        return {'_': [to_schemaless_avro(i, types_to_str, recurse=recurse) for i in o]}

    return o


def from_schemaless_avro(o, schema_gen=None, schemaless_pathes=[], recurse=False):
    """
    Converts a nested data structure in schemaless_avro format into a python nested
    data structure.
    schemaless_avro format is conforming to the avro schema generated by schemaless_avro_schema()
    For fastavro::
        records = [from_schemaless_avro(record)
    ...
    if schemaless_pathes:
        ...
        for record_with_setter in get_schemaless_records(o, schemaless_pathes):
            record = record_with_setter[0]
            setter = record_with_setter[1]
            record = from_schemaless_avro(record, recurse=True)
            setter(record)  # pylint: disable=E1102
        return o
    if not recurse:
        return o
    ...
    if isinstance(o, Mapping):
        o = o['_']
        if isinstance(o, Mapping):
            return {k: from_schemaless_avro(v, recurse=recurse) for k, v in o.items()}
        if isinstance(o, Iterable):
            return [from_schemaless_avro(i, recurse=recurse) for i in o]
